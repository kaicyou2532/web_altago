...
GRANT SELECT ON altago.messages TO anon, authenticated;
GRANT INSERT, UPDATE ON altago.messages TO authenticated;
GRANT SELECT ON altago.applications, altago.users TO anon, authenticated;
ALTER TABLE altago.messages ENABLE ROW LEVEL SECURITY;

-- 応募に紐づく公開チャットの見出しを誰でも取得できるようにする。
DROP POLICY IF EXISTS "applications: public read" ON altago.applications;
