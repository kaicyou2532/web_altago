...
export default function ApplicationChatPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (active) setError('公開チャットを表示できませんでした。');
      } finally {
  ...
  const canSend = !!user && (isClient || user.id === application.runnerId);
  const partnerName = isClient
    ? application.runnerName
    : user?.id === application.runnerId
      ? '依頼者'
      : `${application.runnerName}と依頼者`;
      ...
      <header className="sticky top-16 z-10 border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-3xl items-center gap-3 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-gray-900">{partnerName}の公開チャット</p>
          ...
          {canSend ? (
            <div className="flex gap-2 border-t border-gray-100 p-3">
              <textarea
                ...
                rows={2}
                maxLength={2000}
                placeholder="公開メッセージを入力（Shift+Enterで改行）"
            ...
            <div className="border-t border-gray-100 bg-gray-50 px-4 py-3 text-center text-xs text-gray-500">このチャットは公開されています。投稿できるのは依頼者と応募者のみです。</div>
          )}
        </section>
      </main>
    </div>
